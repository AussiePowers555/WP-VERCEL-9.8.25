  ...
  try {
    ...
    for (const caseItem of cases) {
      try {
        ...
        if (caseItem.id) await DatabaseService.deleteSignatureTokensByCase(caseItem.id);
        
        // Delete any digital signatures
        if (caseItem.id) await DatabaseService.deleteDigitalSignaturesByCase(caseItem.id);
    ...
    const deletedCount = await (DatabaseService as any).deleteAllCases?.();
