...
export default function ContactsPage() {
    ...
    const handleAddContact = async (contact: Omit<Contact, 'id'>) => {
        ...
        try {
            // Use the API directly to get workspace and user info
            const response = await cookieForwardFetch('/api/contacts', {
                method: 'POST',
                headers: {
            ...
            // If workspace and user were created, show credentials modal
            if (result.workspace && result.user && result.tempPassword) {
